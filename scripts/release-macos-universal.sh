...
PLUGIN_NAME="xObsBrowserAutoRefresh"
VERSION=$(grep -o '<AssemblyVersion>[^<]*' "$SRC_DIR/xObsBrowserAutoRefresh.csproj" | sed 's/<AssemblyVersion>//')
...
# so that extracting the tar gives: xObsBrowserAutoRefresh/ (containing xObsBrowserAutoRefresh.plugin + data/)
...
if command -v pkgbuild &>/dev/null; then
  pkgbuild \
    ...
    --identifier "com.yorvex.xobsbrowserautorefresh" \
    --version "$VERSION" \
...
if command -v pkgbuild &>/dev/null; then
  ...
  INSTALLER_IDENTIFIER="com.yorvex.xobsbrowserautorefresh"
...
#!/bin/bash
# Uninstaller postinstall script for xObsBrowserAutoRefresh
